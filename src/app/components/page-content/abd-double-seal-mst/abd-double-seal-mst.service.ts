...
export class AbdDoubleSealMstService {
  ...
  getOtherComponentModel()  {
    return this.http.get<Array<AbdDoubleSealMstSearchModel>>(this.abdDoubleSealMst).pipe( retry(3), catchError(this.handleError) );
  }
  //按条件查询
  getOtherComponentModelWithCondition(searchCondition: AbdDoubleSealMstSearchModel) {
    return this.http.post<Array<AbdDoubleSealMstSearchModel>>(this.abdDoubleSealMst+"searchWithCond", this.makeSearchCondition(JSON.stringify(searchCondition)), httpOptions).pipe( retry(3), catchError(this.handleError) );
  }
...
deleteOtherComponentModelTableData( bumptypeList: String[]) {
  return this.http.delete<any> (this.abdDoubleSealMst + bumptypeList.toString()).pipe( catchError(this.handleError) );
}


//保存（插入）数据至数据库
saveOtherComponentModelTable(currentBumpInfo: any) {
  return this.http.post<any>(this.abdDoubleSealMst+"save", this.checkInput(JSON.stringify(currentBumpInfo)), httpOptions).pipe( retry(3), catchError(this.handleError) );
}
